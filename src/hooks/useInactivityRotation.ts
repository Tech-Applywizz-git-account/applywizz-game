 ...
 */
export const useInactivityRotation = (options: InactivityRotationOptions = {}) => {
  ...
  const rotationSequence = [
    ...
    { path: '/leaderboard', state: { activeTab: 'team' }, displayName: 'Leaderboard Team' },
    { path: '/leaderboard', state: { activeTab: 'individual' }, displayName: 'Leaderboard Individual' }
  ];
  ...
  const resetInactivityTimer = () => {
    ...
    timeoutRef.current = setTimeout(() => {
      ...
      currentRouteIndexRef.current = (currentRouteIndexRef.current + 1) % rotationSequence.length;
